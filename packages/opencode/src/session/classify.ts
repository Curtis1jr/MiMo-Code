...
export function classifyAssistantStep(input: {
  ...
  // 0. Leaked tool-call marker: the provider emits junk text ("call:", "code",
  ...
  if (
    ...
    const hasLeakedToolcallMarker = input.parts.some((part) => {
    ...
    if (hasLeakedToolcallMarker) return { type: "leaked-toolcall-marker" }
  }
